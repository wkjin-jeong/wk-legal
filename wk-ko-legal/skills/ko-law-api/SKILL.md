...
일부 응답은 4자리 padding을 요구하므로, 실패 시 자릿수를 바꿔 재시도한다.
...
- **OC 누락 / 잘못된 OC**: 응답이 비거나 오류 페이지가 반환된다. 환경변수 또는 `--oc` 인자를 확인한다.
- **검색 결과 0건**: 띄어쓰기·괄호·약칭을 바꿔가며 재시도한다(예: "전자금융거래법" / "전자금융거래법(약칭)").
- **JO 인코딩 실패**: 6자리/4자리/zero-padding 변형을 시도. 그래도 실패하면 본문 전체를 받아 사용자측에서 해당 조문을 발췌한다.
- **HTML 응답으로 받고 싶을 때**: `--type HTML` 사용. 단, 인용·발췌가 필요할 때는 XML이 더 안정적.
- **API 호출 횟수 제한**: 법제처 정책상 일일 한도가 있을 수 있다. 같은 세션 내 동일 식별자에 대한 중복 호출은 피한다.
