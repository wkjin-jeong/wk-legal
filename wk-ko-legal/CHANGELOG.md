...
위키 문체 재검증(2026-08-11, 전 코퍼스 331건 — `서면가이드/_자료/정량분석_20260811_문체.md`) 후속 추종. 2.4.7의 자문 사무소 골격 우선 게이트는 유지하고, 같은 날 신설·verified된 문체 자문특칙과의 정합만 보강. 스킬 구성 변동 없음.

- **자문 SKILL.md §4.1 병용 절에 문체 자문특칙 연계 추가**: `서면가이드/문체/자문특칙.md`(결론 기본 어미 '~(것으로) 보입니다' 체계·'다만' 승격 접속어·개진문/결어 고정 정형·조문 박스(■)·리스크 어휘 서열·'본 건' 띄어쓰기)를 05-문체 가이드·공통 문체 규범보다 우선 적용
- **LLM-wiki-연동-정책 §4.1**: 문체 가이드 열거에 자문특칙 추가(공통·형사특칙·자문특칙)
- **자문 05-문체-인용-가이드 어미 지침 교체(자문특칙 정합)**: 종전 "'보입니다'·'사료됩니다' 남용은 판단 가치를 깎는다(결론에서도 한정적)"가 verified 자문특칙(결론 기본 어미 '보입니다' 223회/23건, '판단됩니다' 1회 희소)과 정면 충돌 — 결론 기본 어미 '보입니다'·'판단됩니다/생각됩니다' 금지·'사료됩니다' 보수적 권고 전용·사실 진술 완곡 금지로 재작성, §1.1 결론 예문도 "적용되는 것으로 보입니다"로 교체(종전 "보아야 할 것입니다"는 위키 공통 §10 '~라 할 것입니다' 지양과 상충)
- **어미 예시 잔존 정합(전수 grep)**: SKILL.md §4.5 "결론은 명확히" 예시('판단됩니다' → '것으로 보입니다' + 금지 주석), 04-쟁점분석-방법론 §3.1 결론 어조 표준표 재작성('라 할 것입니다'·'판단됩니다'·'보아야 할 것입니다' 제거, '보입니다' 기본·'사료됩니다' 보수적 권고 전용·'여지가 있습니다' 반영 + 금지 각주), §7 함정 예문 어미 교체
- 참고: 2.4.7이 쓰던 자문 카탈로그 필드명 `의뢰인`·`부제`는 카탈로그 실물과 일치함을 확인(위키 문서 3곳의 `제목` 표기가 오기 — 위키 쪽에서 `부제`로 정정)
- plugin.json 2.4.9 (스킬 구성 변동 없음)

## 2.4.8 — 2026-08-11

drafting 4종 입력자료(첨부 파일) 섹션에서 `.docx`를 입력 가능 형식으로 승격 — .hwpx와 같은 XML 기반 zip 패키지(`word/document.xml`, 텍스트 `<w:t>`)이므로 배제 근거가 없던 비대칭 해소(사용자 지시). 배제 대상은 바이너리 포맷인 구버전 `.hwp`만 남김.

- **민사·자문(상세 bullet형)**: 형식 도입문에서 워드 배제 문구 제거(.hwp만 배제·변환 요청 목록에 .docx 추가), `.hwpx` bullet 아래 `.docx` 추출 bullet 신설 — `word/document.xml` 본문, 표는 본문 내 `<w:tbl>`, 각주(`word/footnotes.xml`)·머리글/바닥글(`word/header*.xml`·`word/footer*.xml`)은 별도 파일 누락 확인, docx 전용 skill 우선 활용·실패 시 변환본 요청 폴백. 자문은 계약서·약관 특성상 **추적 변경(tracked changes)·메모 존재 시 사용자 통지** 추가
- **형사·행정(압축 문단형)**: 같은 내용을 기존 한 문단 문법에 맞춰 반영
- **형식 열거 동기화**: 민사 description(.md·.pdf·.hwpx·.docx·이미지)·민사/자문 작업 모드 (B)의 첨부 형식 열거에 .docx 추가
- 검증: 합성 .docx 추출 스모크 테스트(레시피 실측) + "docx 배제" 문구 잔존 0건 grep
- plugin.json 2.4.8 (스킬 구성 변동 없음)

## 2.4.7 — 2026-08-11

LLM-wiki 2026-08-11 증보(서면DB 2차 반입 119건·누적 331건, 자문 분야 신설 26건, 가이드 3종 신설·2종 증보, 인용규약 개정) 추종. 플러그인은 포인터 설계라 가이드 내용 변경은 자동 반영 — 이번 개정은 (a) 하드코딩된 구조 사실(분야 열거·카탈로그 필드·가이드 존재 예시)의 갱신, (b) 자문 스킬에 부재하던 위키 게이트 신설, (c) 민사 스킬 대상 확장(1안 — 사용자 채택)이다.

- **LLM-wiki-연동-정책 갱신**: §2 서면DB 설명(분야 4개 체계·자문의견서 포함) / §5.1 분야 열거에 자문 추가 + 자문 전례는 doc_id에 의뢰인명이 없으므로 전용 필드 `의뢰인`·`부제`로 좁힌다는 검색 요령 / §5.3 방화벽 실명 범위를 **법인 의뢰인명(`의뢰인` 필드)까지 확장**(위키 CLAUDE.md와 정합) / §5.5 신설 — 서면DB md는 `text_status: auto-extracted` 미검수 추출본이므로 직접 재사용 시 검수, 의심 구간은 `source_raw`가 가리키는 `_raw/` 원본 재검증(문헌 §3 오탈자 주의와 동형)
- **자문 스킬 위키 연계 신설(ko-legal-advisory-drafting)**: 기존 위키 접점이 즐겨쓰는판례 선확인·§8 포인터뿐이던 공백 해소. §4.1에 "사무소 골격 우선" 병용 문단 신설 — 존재 게이트 통과 시 `서면가이드/구조/자문의견서.md`(verified 26건 실측)의 골격 「레터헤드→개진문→다 음→【검토의 요지】→사안의 개요 및 질의의 요지→질의별 검토(질의문 복기)→말미 유보문언」이 스킬 6단 표준 구성보다 우선(자문에는 법정 형식요건이 없어 정책 §4.3 이원 기준상 사실상 전면 위키 우선), 스킬 템플릿은 폴백으로 강등. 사무소 정형 문언은 위키에서 읽어 쓰되 플러그인 저장소로 복사 금지(정책 §1). §4.5 유보 강도 3단(단정형·의견형·유보형) 연결 — "결론 회피 금지" 원칙과의 관계 명시(유보형도 리스크 명시+대안 제시 마감). §5.2 검토 관점 7(문체·표기)에 사무소 정형 대조 추가. references/01-템플릿 서두에 폴백 게이트 문구
- **민사 스킬 대상 확장(1안)**: description·§1에 민사 신청 서면(보전처분·집행·담보·절차 신청) 추가 — 신설 `보전집행신청서.md`(verified 11건)·증보 `절차신청서.md` 가이드가 어느 스킬 대상도 아니어서 트리거 경로가 없던 공백 해소. §4.2 병용 문단에 소장 가이드 예시(청구취지 정형·청구원인 3형·결어 정형)와 신청서군 가이드 연결(수록 유형은 서면가이드 인덱스가 기준), §4.6 신청 서면 특칙 신설(신청취지→신청이유→소명방법 골격, '소갑' 증거 부호, 별지 특정 확인, 위키 부재 시 템플릿 부재 고지 폴백), references/01-소장-template에 위키 소장 가이드 포인터(법정 형식요건은 템플릿·체크리스트 우선 유지)
- **낡은 예시 정정**: 행정 §4.3 "템플릿·가이드가 없는 유형(촉탁·기일변경 등)" — 절차신청서 가이드가 해당 유형을 커버하는 현재 사실과 어긋나던 문구를 절차신청서 준용으로 정정. 형사 §4.2 가이드 고정 열거를 "인덱스에서 고른다(열거는 현재 수록 기준)" 형태로 전환해 위키 성장 시 드리프트 차단 + 절차 신청서류 연결
- **E2E 실측(신설 연동 경로, 2026-08-11)**: ① 존재 게이트+루트 인덱스 선독 ② 서면가이드 인덱스→자문 가이드 front-matter `검증상태: verified` 확인 ③ 카탈로그 검색 실측 — `분야: 자문`+`부제` grep으로 동일 쟁점군(신용정보) 전례 1건 적중, active 26/superseded 0 필터 정상 ④ 합성 사례 사무소 골격 스켈레톤 산출(scratchpad) — 레터헤드·'참고하시기를' 개진문·검토의 요지 박스(질의당 1문장)·질의문 복기 소제목·유보 강도 의견형/유보형+대안 마감·말미 유보문언 전건 요소 재현, 가공 의뢰인만 사용 ⑤ 방화벽 grep — 플러그인 저장소 내 실제 의뢰인명·유보문언·jamun doc_id 앵커 0건. 판례 검색·실무제요 단계는 2.4.5 E2E 실측 유효(경로 무변경)로 범위 외
- **위키 문서 결함 발견(플러그인 외 — 사용자 조치 필요)**: 위키 인용규약 §2·서면가이드 인덱스 규칙 2의2·자문 가이드 §5는 자문 전용 필드를 `의뢰인`·`제목`으로 기재하나, 실측상 카탈로그 자문 26건 전부 `제목` 필드가 없고 제목 원문이 `부제` 필드에 수록 — 플러그인은 실측(`부제`) 기준으로 반영했고 위키 측 표기 정정 필요
- plugin.json 2.4.7 (스킬 구성 변동 없음, 민사 스킬 description 확장)

## 2.4.6 — 2026-08-10

drafting 4종 SKILL.md 최상위 판례 검색 지시문(총론·검토 모드)의 lbox-case-search 하드코딩 중립화 — 2.4.4가 "재해석 조항 보호"를 근거로 남긴 구간의 영향 재평가에서, 정책 참조 포인터의 주제 열거("검색 일괄화·원문 인용·검색 실패 폴백·판례 표기")에 **검색원 선택이 누락**되어 §1.1을 참조할 문면상 근거가 없고 명령형 lbox 지정(사건유형·대법원 우선 파라미터 포함)이 유일 지시로 남는 구조적 공백 확인(특히 위키 부재 환경의 신규 초안 경로 — 즐겨쓰는판례 선확인이 생략되면 검색 전에 정책 파일을 열 계기가 없음). 4파일 19개소, 스킬 구성 변동 없음.

- **총론 4곳 재구성(§1.1 요지 인라인화)**: "초안 착수 전 쟁점 목록 확정 → lbox-case-search 호출 → 한 번의 브라우저 세션 연속 수행" 문단을 "쟁점 목록을 핵심/부수로 분류 → **판례 검색 스킬(핵심 쟁점 lbox·bigcase 병렬, 부수 쟁점 bigcase 단독 — 정책 1.1)** 호출 → 한 번의 브라우저 세션(**사이트별 탭 1개**) 연속 수행"으로 재작성 — 검색원 분기를 정책 파일 왕복 없이 SKILL 본문에서 직접 지시, "한 번의 세션" 문구와 병렬 검색의 표면상 충돌도 해소. 행정·형사의 사건유형("행정"/"형사")·대법원 우선 파라미터는 중립 표기로 유지
- **포인터 주제 열거에 '검색원 선택' 추가(4곳)**: "검색 일괄화·…의 상세 원칙은 판례-인용-정책.md를 따른다" → "**검색원 선택**·검색 일괄화·…" — 재해석 조항(§1)이 닿는 문면상 경로 확보
- **검토 모드 4곳 중립화**: 민사·행정·형사 검토 체크 "인용이 약하면 lbox-case-search로 보강"·자문 "추정 인용은 모두 lbox로 검증·교체" → 판례 검색 스킬 + 검색원 선택 1.1 참조
- **인접 배타·단독 문구 정리**: 민사·자문 "lbox에서 확인한 사건번호·선고일자·판결 종류만 인용" 및 자문 4.6 자체 점검 "법령·판례가 lbox에서 확인된 것인가" → 판례 검색 스킬(법령은 ko-law-api로 분리 명기), 자문 4.1 3단계·4.4 검색어 예시 표기 중립화, 자문 "금융감독규정·유권해석도 lbox에서 일부 검색" → 결정례·유권해석 bigcase 우선(1.1 특성 라우팅) 병기, **행정 총론에 헌재 결정·재결례·유권해석 bigcase 우선 문장 추가**(lbox 사건유형에 헌법이 없어 생기던 행정 특유 공백 해소)
- 영향 평가 근거(2.3.2·2.4.5 실측 인용): 상위 30건 겹침 1/6 — 단독 검색 시 반대편 약 25건 미노출, 부수 쟁점 왕복 lbox ~18회 vs bigcase ~6회(약 3배), 크로스 폴백 미작동 시 회피 가능한 `[판례 미확인]` 발생 여지, 단일 검색원 원문 의존의 오식 인용 위험(2.4.5 bigcase 오식 실측의 대칭 사례)
- 이로써 drafting 4종 SKILL.md의 lbox 단독 지시 잔존 0건(전수 grep 확인) — 남은 lbox 단독 표기는 lbox-commentary-search 폴백 참조(lbox 전용 스킬이라 정당)·lbox 계열 스킬 자체 파일·evals.json 사이트 명시 프롬프트뿐
- plugin.json 2.4.6 (스킬 구성 변동 없음)
